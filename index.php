 ...
 * Sorted by datetime (latest activity first) so newest posts appear at top
 * Each event includes event_time showing when it actually occurred
 */
function getEventsFromDb(array $filters = [], int $limit = 500, int $offset = 0): array {
    ...
    $query = "SELECT raw_data, event_time, publish_time, last_updated, datetime FROM events WHERE 1=1";
    $params = [];
    ...
    // Sort by datetime (latest activity first) so newest posts/updates appear at top
    // The UI shows event_time (when it happened) vs datetime (when published)
    ...
    while ($row = $stmt->fetch()) {
        ...
        // Check if event_time differs significantly from datetime (>30 min)
        // This indicates the event happened earlier than when it was published
        $event['was_updated'] = false;
        if ($row['event_time'] && $row['datetime']) {
            try {
                $eventT = new DateTime($row['event_time']);
                $publishT = new DateTime($row['datetime']);
                $diffMinutes = abs($publishT->getTimestamp() - $eventT->getTimestamp()) / 60;
                $event['was_updated'] = $diffMinutes > 30;
            } catch (Exception $e) {}
        }
        $events[] = $event;
    }
...
if (isset($_GET['ajax'])) {
    ...
    if ($_GET['ajax'] === 'events') {
        ...
        $formatted = array_map(function($e) {
            // Use datetime (API/publish time) for main display - this is what we sort by
            $publishTime = $e['datetime'] ?? date('Y-m-d H:i:s');
            $date = formatDate($publishTime);

            // Show "inträffade HH:MM" if event_time differs significantly from datetime
            $occurredAt = null;
            if (!empty($e['was_updated']) && !empty($e['event_time'])) {
                $eventDate = formatDate($e['event_time']);
                $occurredAt = $eventDate['time']; // Just show time like "09:49"
            }
            ...
            return [
                ...
                'occurredAt' => $occurredAt, 'wasUpdated' => !empty($e['was_updated']),
            ];
        }, $events);
                ...
                <div class="events-grid" id="eventsGrid" style="<?= $currentView !== 'list' ? 'display:none' : '' ?>">
                    ...
                    <?php else: ?>
                        <?php foreach ($initialEvents as $i => $event):
                            // Use datetime (publish time) for main display - this is what we sort by
                            $publishTime = $event['datetime'] ?? date('Y-m-d H:i:s');
                            $date = formatDate($publishTime);
                            $type = $event['type'] ?? 'Okänd';
                            ...
                            // Show "inträffade" when event_time differs significantly from datetime
                            $wasUpdated = !empty($event['was_updated']);
                            $occurredTime = $wasUpdated && !empty($event['event_time']) ? formatDate($event['event_time'])['time'] : null;
                        ?>
                            <article class="event-card" style="animation-delay: <?= min($i * 0.02, 0.2) ?>s">
                                <div class="event-card-inner">
                                    <div class="event-date">
                                        ...
                                        <?php if ($occurredTime): ?>
                                            <div class="occurred-indicator" title="Händelsen inträffade <?= $occurredTime ?>">inträffade <?= $occurredTime ?></div>
                                        <?php endif; ?>
                                    </div>
